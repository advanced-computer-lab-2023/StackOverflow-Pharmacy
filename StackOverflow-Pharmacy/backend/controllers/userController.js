...
const getChats = async (req, res) => {
  try {
    ...
    if(user.role==='Patient'){
      ...
      const patientChats = await Chat.find({ $or: [{ firstID: patientId }, { secondID: patientId }] });
      ...
      const newChats = pharmacists.filter(pharmacist =>
        !patientChats.some(chat =>
          chat.firstID.equals(patientId) && chat.secondID.equals(pharmacist._id)
        )
      );
    ...
    if(user.role==='Pharmacist'){
      ...
      const pharmacistChats = await Chat.find({ $or: [{ firstID: patientId }, { secondID: patientId }] });

      // Identify pharmacists with whom the patient doesn't have a chat
      const newChats = patients.filter(patient =>
        !pharmacistChats.some(chat =>
          chat.firstID.equals(patientId) && chat.secondID.equals(patient._id)
        )
      );
    ...
    if (patientId) {
      ...
      const chats = await Chat.find({ $or: [{ firstID: patientId }, { secondID: patientId }] });
...
//     if (_id) {
//       const chat = await Chat.findOne({ firstID: _id,secondID:secondID }).select('secondID');
...
//   }
// };
// const getNamesFromChats = async (chats) => {
//   const names = [];
//   for (const chat of chats) {
//     try {
//       const secondUserId = chat.secondID;
//       const user = await User.findOne({ _id: secondUserId }).select('username');
//       if (user) {
//         names.push(user.username);
//       }
//     } catch (error) {
//       console.error(`Error fetching name for secondID ${chat.secondID}: ${error.message}`);
//     }
//   }
//   return names;
// };
...
//     const secondID = req.body;
//     if (firstID) {
//       const chat = await Chat.findOne({ firstID,secondID});
//       res.status(200).json({chat});
//     } else {
//       res.status(500).json({ error: "Failed to get user ID" });
//     }
...
const sendMessage = async (req, res) => {
  ...
  try {
    ...
    const text = req.body.text; // Fix the typo here

    if (chat) {
      // const message = await Message.create({ sender: chat.firstID, text: text });
      const message = { sender: chat.firstID, text: text }
